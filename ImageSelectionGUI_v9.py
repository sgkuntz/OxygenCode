...
possibleEventList = ['posterior_gap','pole_bud_appears','nuclei_at_periphery','pole_cells_form','yolk_contraction','cellularization_begins','membrane_reaches_yolk',
				'pole_cells_migrate','cephalic_furrow_formation','pole_cell_invagination','cephalic_furrow_reclines','amnioproctodeal_invagination','amnioproctodeal_invaginationA','amnioproctodeal_invagination_begins','amnioproctodeal_invagination_ends',
...
imageDirectory = '/Volumes/Coluber/Imaging/'
...
class App: # define the App, which should record which button is pressed
	def __init__(self, master, timepoint, animal, event, condition, date, position, nameA, nameB, timeDepth, tNameLength, dilation):
		...
		self.jump_next.grid(row=1, column = 5)

		self.jump_previous = Button(text="<<", padx = 20, pady = 15, command=self.jumpPrevious)
		self.jump_previous.grid(row=1, column = 1)

		self.jump_next = Button(text=">>>", padx = 20, pady = 15, command=self.jumpNextNext)
		self.jump_next.grid(row=1, column = 6)

		self.jump_previous = Button(text="<<<", padx = 20, pady = 15, command=self.jumpPreviousPrevious)
	...
	def jumpNextNext(self):
		self.frame.quit()
		global statusCall
		statusCall = 'jumpnextnext'		
	def jumpPreviousPrevious(self):
		self.frame.quit()
		global statusCall
		statusCall = 'jumppreviousprevious'
	def noMatches(self):
		self.frame.quit()
			...
			try:
				...
				while statusCall <> 'proceed':
					...
					while statusCall == 'jumpprevious':
						...
						app = App(root, estTime, animal, devEvent, condition, animal[:8], animal[9:], firstFilename[:21], filenameB, numberOfImages, tCallLength, scaling)
						root.mainloop()
					while statusCall == 'jumpnextnext':
						estTime += numberOfImages*25
						app = App(root, estTime, animal, devEvent, condition, animal[:8], animal[9:], firstFilename[:21], filenameB, numberOfImages, tCallLength, scaling)
						root.mainloop()
					while statusCall == 'jumppreviousprevious':
						if estTime > numberOfImages*26:
							estTime -= numberOfImages*25
						app = App(root, estTime, animal, devEvent, condition, animal[:8], animal[9:], firstFilename[:21], filenameB, numberOfImages, tCallLength, scaling)
						root.mainloop()
