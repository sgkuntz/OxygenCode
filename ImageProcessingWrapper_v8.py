...
for (dirpath, dirnames, filenames) in os.walk("../../../Volumes/Coluber/Imaging/"): # go to ../../Volumes/Agkistrodon/Imaging
	...
	if 'aligned' not in dirpath:
		for filename in filenames:
			if '_t000_' in filename: # search for all file names with */*/*_t000_* and */*/*_t0000_*
				# print dirpath
				fullDate = dirpath[33:41] 
				abbDate = fullDate[:2]+fullDate[3:5]+fullDate[6:]
				pos = dirpath[45:] # should this be 50?
				...
				fullDate = dirpath[33:41]
				abbDate = fullDate[:2]+fullDate[3:5]+fullDate[6:]
				pos = dirpath[45:] # should this be 49?
				fileNamesData.append((fullDate, abbDate, pos, filename))
...
outputFileTestedCopy = open('TimeLapseInstructions_run_' + str(sys.argv[2]) + '.txt','w')
		...
		if shortDate == abbrDate:
			if translatedPosition == Pos:
				...
				if len(fileName) == 28 or len(fileName) == 29: # no z or channel info
					...
					print "error in file name", fileName, shortDate
				if timeDilation == '':
					scaling = 1
				else:
				...
				if (fullDate, Pos) not in alreadyCompletedDates:
					...
					outputFileTestedCopy.write(str(fullDate) + '\t' + 'Pos' + str(Pos) + '\t' + str(fileName) + '\t' + str(orientation) + '\t' + str(filenameStructure) + '\t' + str(mry) + '\t' + str(trachea) + '\t' + str(species) + '\t' + str(temperature) + '\t' + str(scaling)  + '\t' + str(z) + '\n')
				else:
					print fullDate, Pos